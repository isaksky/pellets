...
func (s *Scheduler) requireOwnership(ctx context.Context, run storage.ExecutionRun) error {
	...
	if p.ImplementationRevision != run.ImplementationRevision || p.Status != domain.PelletInProgress || p.Workspace == nil || p.Workspace.ID != run.WorkspaceID || p.Title != run.PelletTitle || p.Description != run.PelletDescription {
