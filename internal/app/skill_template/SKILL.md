...
- The internal Codex adapter checks the installed runtime's stable app-server schema and uses local stdio. It is not yet a public runner command. Treat unsupported capabilities as an actionable runtime compatibility error; do not substitute a separate model/tool loop or invent command flags.
