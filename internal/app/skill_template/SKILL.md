...
- When the server delegates an already selected pellet, work only on that exact pellet; do not run `next` or `start-next` or advance the queue yourself. Implement, verify, commit, then close that exact pellet through `pl` within the authorized scope. Turn-end or closed status alone cannot prove completion: the server also validates the exact turn, new result commit, durable evidence, and process cleanup.
