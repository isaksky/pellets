...
func TestDatastarLiveResponseBundlesRelatedRegions(t *testing.T) {
	...
	if _, err := fixture.application.CreatePellet(context.Background(), fixture.projects[0], storage.NewPellet{Title: "Review live task", Kind: domain.PelletReviewCheckpoint, ReviewTargets: []domain.PelletReference{pellet.Reference}}); err != nil {
		t.Fatal(err)
	}
	...
	for _, target := range []string{"project-counts", "area-tabs", "project-record", "project-drawer", "breadcrumbs", "run-dashboard", "workspace-groups", "tasks-title", "task-list", "inspector-host"} {
		if strings.Count(body, "data: selector #"+target+"\n") != 1 {
			t.Errorf("expected one patch for %s", target)
		}
	}
	if strings.Contains(body, "data: mode replace") || !strings.Contains(body, `class="footer-counts">2 active in project</span>`) || !strings.Contains(body, `reviews">2</small>`) || !strings.Contains(body, `Memories <small>0</small>`) || !strings.Contains(body, `2 open · 0 in progress`) || !strings.Contains(body, `1 pellet · 1 review`) {
