...
function render() {
  ...
  state.drafts.forEach((draft,index) => {
    ...
    const rowError=node.querySelector('.plan-row-error');
    const missing=!draft.title.trim()?'Missing title':(!draft.description.trim()&&!(draft.acceptance||'').trim())?'Missing details':'';
    rowError.hidden=!missing;
    rowError.textContent=missing;
    rowError.title=missing==='Missing title'?'Enter a pellet title.':'Add a description or acceptance criteria.';
  ...
  refreshComponents(panel);
  refreshDescriptions(panel);
