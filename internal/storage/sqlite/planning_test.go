...
func TestPlanningMigrationPreservesExistingQueue(t *testing.T) {
	...
	pellet, err := scanPellet(legacy.QueryRowContext(ctx, strings.NewReplacer("t.evidence_title", "NULL", "t.evidence_description", "NULL", "p.group_record_id", "NULL", "p.model", "NULL", "p.reasoning_effort", "NULL").Replace(pelletSelect)+" WHERE p.project_id=? AND p.number=1", project.ID))
	if err != nil {
		t.Fatal(err)
	}
