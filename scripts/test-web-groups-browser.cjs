...
(async () => {
  ...
  const revision=await dialog.locator('form[id^=edit-group-context] [name=revision]').inputValue();
  for(let i=0;i<3;i++) {
    await refresh();
    ...
    assert.equal(await dialog.locator('form[id^=edit-group-context] [name=revision]').inputValue(),revision);
  }
  page.once('dialog', d=>d.dismiss()); await dialog.getByRole('link',{name:'Close inspector',exact:true}).click();
  ...
  const nameField=dialog.getByRole('textbox',{name:'Group name',exact:true});
  const headerBefore=await dialog.locator('header').boundingBox();
  await edit(); await field.fill('Unsaved context survives rename');
  await nameField.fill('Renamed browser group');
  await refresh(); assert.equal(await nameField.inputValue(),'Renamed browser group');
  assert.equal(await field.isEnabled(),true);
  await nameField.press('Enter');
  await until(async()=>cli('group','show',String(created.id)).name==='Renamed browser group','Enter saves name');
  await until(async()=>!(await dialog.locator('.group-title-editor').getAttribute('data-pending')),'Rename completes');
  assert.equal(await field.inputValue(),'Unsaved context survives rename');
  const headerAfter=await dialog.locator('header').boundingBox();
  assert.equal(headerAfter.height,headerBefore.height,'Editing title preserves header height');
  await nameField.fill('Canceled title'); await nameField.press('Escape');
  assert.equal(await nameField.inputValue(),'Renamed browser group');
  assert.equal(await dialog.evaluate(el=>el.open),true,'Escape only cancels title');
  await nameField.fill('Blur saved title'); await field.focus();
  await until(async()=>cli('group','show',String(created.id)).name==='Blur saved title','Blur saves name');
  await until(async()=>!(await dialog.locator('.group-title-editor').getAttribute('data-pending')),'Blur completes');
  await nameField.fill('My stale name');
  cli('group','rename',String(created.id),'Concurrent name');
  await nameField.press('Enter');
  await dialog.locator('#group-name-error').waitFor({state:'visible'});
  assert.equal(await nameField.inputValue(),'My stale name');
  ...
  await groupLink('Concurrent name').click();
  await field.waitFor({state:'visible'});
