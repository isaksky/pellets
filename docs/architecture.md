...
SQLite documents that [`PRAGMA data_version`](https://sqlite.org/pragma.html#pragma_data_version) values are meaningful only when two values come from the same connection. A change means that some other connection committed; it does not identify a table, row, order, or number of commits. The web writer is intentionally a different connection, so both its commits and commits from CLI processes change the pinned monitor's observation. Rollbacks and reads do not. The monitor therefore treats a changed value only as an invalidation signal, coalesces bursts, and broadcasts a bounded `pellets-invalidate` SSE message containing no row data, database path, or capability. Each visible Datastar fragment then performs its normal authoritative GET. Native `EventSource` reconnect plus slower Datastar polling and initial loads repair missed messages.
...
Datastar 1.0.3 is an embedded ES module imported by `app.js`. Template `data-on` and `data-on-interval` attributes invoke application actions that delegate to Datastar's built-in GET/POST actions. Requests identify a fixed fragment through `Pellets-Target`; responses use `datastar-patch-elements` followed by an `_webResult` signal containing status, canonical URL, and refresh intent. Datastar requests receive HTTP 200 for successful and actionable conflict/validation responses because the client only consumes HTTP 200 bodies. The signal retains application status 200/201/409/422; ordinary requests retain their original HTTP status. Other errors keep their HTTP status and never patch the UI. Form payloads remain URL-encoded.

The browser coordinates requests per destination, rejects stale responses after navigation, and protects dirty inspectors and open disclosures both before a request and before its patch. Conflict drafts remain visible during automatic refresh. Canonical URLs are pushed into browser history; Back/Forward reload authoritative HTML after checking for unsaved edits. The application script URL changes for the migration to bypass previously immutable cached JavaScript; only the pinned Datastar asset remains immutable, while application assets require revalidation.
