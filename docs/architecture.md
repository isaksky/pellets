...
All editable rows carry an opaque SHA-256 version token derived from JSON encoding of the complete authoritative Go row, including identity, nullable values, lifecycle/order/ownership, provenance/approval, and timestamps. The storage method reloads the row and validates the submitted token only after `BEGIN IMMEDIATE`. Mismatch returns the current materialized row and rolls back without a write; the inspector displays the conflict beside the preserved submitted draft. Creation has no pre-existing row token. Memory text replacement deletes the old external-content FTS row, changes authoritative text, and inserts the new FTS row in that same short transaction.

Datastar 1.0.3 is an embedded ES module imported by `app.js`. The backend uses the official `datastar-go` SDK v1.2.2 for SSE headers, event encoding, and flushing. The application retains carriage-return normalization and the no-store response policy; write/flush failures stop a bundle before its completion signal. Template `data-on` attributes invoke application actions that delegate to Datastar's built-in GET/POST actions. One page-level `data-on-interval` provides recovery polling. Requests identify an interaction through `Pellets-Target`; `live` requests refresh all live regions. Successful responses render related regions from one materialized page-data load and emit multiple `datastar-patch-elements` events followed by one `_webResult` signal containing status, canonical URL, and bundle completion. Counts, area tabs, project/workspace metadata, and the task or memory list are included with successful interactions; live refreshes also include the inspector. Stable regions use outer morphing and inspectors use inner morphing; task rows, memory cards, and the task scroll container have stable IDs. Inspector roots and edit forms have record-specific IDs, preventing live input values from carrying between records. Confirmed discard resets the previous form only immediately before an accepted inspector patch, after route and edit-revision guards; this also discards drafts during same-record lifecycle actions while preserving them on failed requests or when newer edits arrive in flight.

Datastar requests receive HTTP 200 for successful and actionable conflict/validation responses because the client only consumes HTTP 200 bodies. The signal retains application status 200/201/409/422; ordinary requests retain their original HTTP status. Other errors keep their HTTP status and do not patch editable content. Missing completion produces visible request-failure feedback while preserving drafts. Foreground requests show a loading status; save buttons are disabled until completion. Form payloads remain URL-encoded and mutation results retain the current sort/filter query.

The browser coordinates one foreground request and one background bundle, rejects stale responses after navigation, and protects dirty inspectors and open disclosures at patch time without discarding unrelated updates. If newer edits reject a foreground navigation’s inspector patch, the rest of that navigation bundle is rejected as well, preserving the original list selection and URL. A foreground interaction supersedes background work; a pending mutation completes before the latest queued navigation/filter intent. Conflict drafts remain visible during automatic refresh. Canonical URLs are pushed into browser history only when the primary requested patch is accepted; Back/Forward reload authoritative HTML after checking for unsaved edits. Only the pinned Datastar asset remains immutable, while application assets require revalidation.

The HTTP mutation surface requires the exact listener `Host`, exact same-loopback `Origin`, a per-process cryptographically random CSRF capability in both a `SameSite=Strict`/`HttpOnly` cookie and form, POST, and `application/x-www-form-urlencoded`. Responses set a restrictive CSP permitting same-origin scripts/styles/connects and a random per-page nonce for Datastar expression compilation (no unsafe-inline or unsafe-eval), disable framing and MIME sniffing, and use `html/template` escaping. Assets are embedded: pinned Datastar 1.0.3 plus its MIT license, repository JavaScript for native EventSource/theme/focus enhancements, and hand-authored CSS. Runtime CDN, font, icon, Node/npm, WebSocket, and additional framework dependencies are absent.
