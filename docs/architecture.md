...
The browser coordinates one foreground request and one background bundle, rejects stale responses after navigation, and protects dirty inspectors and open disclosures at patch time without discarding unrelated updates. A foreground interaction supersedes background work; a pending mutation completes before the latest queued navigation/filter intent. Conflict drafts remain visible during automatic refresh. Canonical URLs are pushed into browser history; Back/Forward reload authoritative HTML after checking for unsaved edits. Only the pinned Datastar asset remains immutable, while application assets require revalidation.
