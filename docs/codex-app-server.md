...
and was checked against the stable JSON schema generated by Codex CLI 0.154.0.
...
[Managed runtime setup, distribution, updates, overrides, and offline behavior](codex-runtime.md)
are part of this boundary. `Start(ctx, Config)` resolves the managed pinned
package by default, or the explicit configured executable,
executes `--version`, rejects versions below the stable 0.154.0 floor, and asks
that same executable to run
...
An empty executable selects the managed pin. Local resolution is captured
separately in `settings.runtime`, not injected into portable default settings.
Nonempty legacy saved paths remain explicit overrides; Resume checks current
settings and preserves the original attempt and conversation.
...
failure, never evidence that unseen work completed. The foreground recorder preserves a failed exact turn's sanitized message in
the durable summary, separately from its error code and process outcome.
Persist turn status/error separately from the process outcome, and use conversation read/resume operations
...
An explicit, optional smoke check validates the selected managed/override runtime version,
