...
- The only listener address is IPv4 `127.0.0.1`. There is no bind-address flag. Omitted `--port` starts at 7419 and increments only when a port is already in use, stopping at the first available port or failing at 65535 without wrapping. Explicit `--port` accepts 1 through 65535 and requires that exact port; explicit canonical port `0` requests an OS-selected available port. Other listener errors stop startup immediately.
